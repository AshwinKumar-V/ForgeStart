...
resolver.define('getCurrentWeather', async (req) => {
  ...
  if(config?.lat && config?.lon) {
    const url = "https://api.openweathermap.org/data/2.5/weather?lat=" + config.lat + "&lon=" +
        config.lon + "&units=metric&appid=" + process.env.OPENWEATHER_KEY;
    const response = await fetch(url)
    if(!response.ok) {
      const errmsg = `Error from Open Weather Map Current Weather Data API: ${response.status} ${await response.text()}`;
      console.error(errmsg)
      throw new Error(errmsg)
    }
